...
import com.saga.outbox.atividade.mapper.ClienteMongoMapper;
...
public class ClienteSagaService {
    ...
    public ClienteResponse criarCliente(ClienteRequest request) {
        ...
        try {
            ...
            //Salva no postgres como pendente (inicia SAGA)
            entity.setStatus("PENDENTE");
            clienteEntityRepository.save(entity);

            //Outbox (registra operação pendente)
            ...
            //Salva no mongo (replica no mongo)
            ClienteMongo mongo = ClienteMongoMapper.toEntity(request);
            clienteMongoRepository.save(mongo);

            //Salva no postgres como confirmadov (confirma SAGA)
            entity.setStatus("CONFIRMADO");
            clienteEntityRepository.save(entity);

            //Salva o outbox (cpnfirma processamento)
            ...
            throw new RuntimeException("Erro na transação distribuída");
        }
    }
    ...
    private void compensar(Long id) {
        ...
        clienteEntityRepository.findById(id)
                .ifPresent(cliente -> {cliente.setStatus("FALHA");
